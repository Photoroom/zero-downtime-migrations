...
mod diff_mode {
    ...
    #[test]
    fn diff_mode_reads_head_content_not_worktree_content() {
        let temp = setup_git_repo();
        let path = temp.path().join("app/migrations/0001_bad.py");
        fs::write(&path, BAD_MIGRATION_NON_CONCURRENT_INDEX).unwrap();
        git_commit_all(temp.path(), "Add unsafe migration");
        fs::write(&path, CLEAN_MIGRATION).unwrap();
    ...
    #[test]
    fn diff_mode_runs_r008_when_application_file_is_deleted() {
        let temp = setup_git_repo();
        fs::write(
            temp.path().join("zero-downtime-migrations.toml"),
            r#"allowed-file-patterns = []"#,
        ...
        git_commit_all(temp.path(), "Add config and model");
        set_base_to_head(temp.path());

        fs::remove_file(temp.path().join("app/models.py")).unwrap();
        fs::write(temp.path().join("app/migrations/0001.py"), CLEAN_MIGRATION).unwrap();
        git_commit_all(temp.path(), "Add migration and remove model");
...
mod multiple_files {
    ...
    #[test]
    fn duplicate_explicit_paths_are_linted_once() {
        let temp = setup_migrations(&[("0001_bad.py", BAD_MIGRATION_NON_CONCURRENT_INDEX)]);
        let path = temp.path().join("app/migrations/0001_bad.py");
        let output = zdm()
            .arg(&path)
        ...
        assert_eq!(diagnostics(&parse_json_output(&output)).len(), 1);
    }
...
mod list_rules {
    ...
    #[test]
    fn list_rules_ignores_invalid_project_config() {
        let temp = TempDir::new().unwrap();
        fs::write(
            temp.path().join("zero-downtime-migrations.toml"),
            "this is not toml = [",
    ...
    #[test]
    fn public_api_remains_available() {
